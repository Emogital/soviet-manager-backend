...
namespace GameServer.Services.Gameplay.Matches
{
    public class MatchService(ILogger<MatchService> logger) : IMatchService
    {
        ...
        private IMatchPlayer[] InitCoopForTwoMatchPlayers(Player[] players)
        {
            ...
            AddMatchPlayer(matchPlayers, players[0], teamId: 1);
            AddMatchPlayer(matchPlayers, players[1], teamId: 1);
            AddMatchPlayer(matchPlayers, new Bot(2), teamId: 2);
            AddMatchPlayer(matchPlayers, new Bot(3), teamId: 2);
            return matchPlayers;
        }
        ...
        private IMatchPlayer[] InitTwoByTwoMatchPlayers(Player[] players)
        {
            ...
            AddMatchPlayer(matchPlayers, players[0], teamId: 1);
            AddMatchPlayer(matchPlayers, players[1], teamId: 1);
            AddMatchPlayer(matchPlayers, players[2], teamId: 2);
            AddMatchPlayer(matchPlayers, players[3], teamId: 2);
            return matchPlayers;
        }
        ...
        private IMatchPlayer[] InitCoopHardcoreMatchPlayers(Player[] players)
        {
            ...
            AddMatchPlayer(matchPlayers, new Bot(2), teamId: 2);
            AddMatchPlayer(matchPlayers, new Bot(3), teamId: 2);
            AddMatchPlayer(matchPlayers, new Bot(4), teamId: 2);
            return matchPlayers;
        }
        ...
        private IMatchPlayer[] InitConfrontationMatchPlayers(Player[] players)
        {
            var matchPlayers = new IMatchPlayer[4];
            AddMatchPlayer(matchPlayers, players[0], teamId: 1);
            AddMatchPlayer(matchPlayers, players[1], teamId: 2);
            AddMatchPlayer(matchPlayers, new Bot(2), teamId: 1);
            AddMatchPlayer(matchPlayers, new Bot(3), teamId: 2);
            return matchPlayers;
        }
        ...
        private void AddMatchPlayer(IMatchPlayer[] matchPlayers, IMatchPlayer player, int teamId)
        {
            matchPlayers[player.Id] = player;
